---
name: skill-designer
description: 技能设计助手：把想法变成结构规范、可直接安装的技能目录，包括带规范 frontmatter 的 SKILL.md、UI 元数据以及可选的 scripts、references、assets。Skill design assistant that turns an idea into a well-structured, installable skill folder. 当用户要求设计、创建、搭建、完善技能时使用，例如「设计一个 skill」「帮我创建技能」「make a skill that does X」「把这个流程做成技能」。Use when a user asks to design, create, build, scaffold, or shape a new skill, or wants to turn a repeated workflow into an installable skill.
---

# Skill Designer · 技能设计师

把用户请求变成规范、可安装的技能目录：收集需求 → 设计方案 → 脚手架生成 → 实现 → 校验 → 迭代。
Turn a user request into a well-structured, installable skill folder: collect requirements, plan, scaffold, implement, validate, iterate.
...
- 安装位置：默认技能目录（~/.codex/skills）。 / Install location: the skills directory (~/.codex/skills) by default.
...
- frontmatter 描述：写清技能做什么和所有具体触发场景（用户说法、文件类型、场景）。这是唯一的触发机制。 / Frontmatter description: state what the skill does and every concrete trigger (user phrasings, file types, scenarios). This is the only trigger mechanism.
...
```bash
python scripts/init_skill.py <skill-name> --path <install-dir> --resources scripts,references,assets --interface display_name="..." --interface short_description="..." --interface default_prompt="..."
```

- 初始化脚本会创建文件夹、SKILL.md 和 agents/openai.yaml（UI 元数据）。 / The initializer creates the folder, SKILL.md, and agents/openai.yaml (UI metadata).
...
- scripts/generate_openai_yaml.py — 重新生成 UI 元数据。 / regenerates UI metadata.
